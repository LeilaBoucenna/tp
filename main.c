...
book * book_search(int n)///search for the book by his id if founded returns it @ else return 0(doesnt exist)
{
  if (HEAD==NULL)
 return NULL;//si la liste est vide donc il n existe pas
  ///si oui liste n est pas vide
    ...
    temp=next_book(temp);; ///si marche supprime
     }
   return NULL;
}
/////////////////////////
...
///!!!!!!!!!!!!do i delete it completely or deincrment the nmb of copy availble until =0 at this moment i delete completly
...
{   bool found=false;
 ...
 {
     if (temp->id==n)//je le supprime et je fais un lien entr prev et next si il est au milleiu sinon cas fin et cas debut
     {  found=true;
...
else
{
    printf("<== Available Books ==>\n\n");
     printf("%-10s %-30s %-40s %-50s\n", "Book id", "Book Name","Book author","Number of copies available");

     while(temp!=NULL)
        {
        printf("%-10d %-30s %-40s %-50d \n",temp->id,temp->title,temp->author,temp->copy);
        temp=temp->next;
       }
...
///first case if the book list is empty we add the first book
  ...
  return;
   ...
   if(HEAD==NULL || TAIL==NULL)///first case if the book list is empty we add the first book
   ...
   {p=book_search(id);
      ...
      else ///il nexister pas avant je cree une nouvelle cellule avec nbr copy =1
      {    /// mais sa doit etre une sorted linked list por le moment elle ne sera pas organiser
...
int main()
{
 ...
 do
 {
    ...
    printf("2-Add book or Return a book.\n");///add and return are not the same,in return i need to ask for the student id and maybe other stuff
    printf("3-Remove a book or Take a book.\n");///but in add i need nothing
    printf("4-Display borrower list.\n");
    ///search book dont forget it
    printf("5-Loan list.\n");
    printf("6-Display statistics of the total number of books and borrowers, active loans, and overdue books.\n\n");
